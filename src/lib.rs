...
mod capture;
mod d3d11;
mod dxgi_duplication_api;
mod encoder;
mod frame;
mod graphics_capture_api;
mod graphics_capture_picker;
mod monitor;
mod settings;
mod window;

use bytemuck::{Pod, Zeroable, cast_slice};
use capture::{CaptureControl, CaptureControlError, Context, GraphicsCaptureApiError, GraphicsCaptureApiHandler};
use dxgi_duplication_api::{DxgiDuplicationApi, Error as DxgiDuplicationError};
use frame::Frame;
use graphics_capture_api::InternalCaptureControl;
use monitor::Monitor;
...
#[pymodule]
fn ray_shot(m: &Bound<'_, PyModule>) -> PyResult<()> {
...
fn bgra_to_rgb(bgra_bytes: &[u8]) -> Vec<u8> {
    ...
    rgb.par_chunks_mut(3).zip(bgra_pixels.par_iter()).for_each(|(rgb_px, bgra)| {
        rgb_px[0] = bgra.r;
        rgb_px[1] = bgra.g;
        rgb_px[2] = bgra.b;
    });
